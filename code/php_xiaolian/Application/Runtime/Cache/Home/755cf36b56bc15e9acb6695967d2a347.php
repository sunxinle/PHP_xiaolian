 ...
 $(function(){
	...
	var buildContactDiv = function(contactlistDivId, roster) {
		...
		for (i = 0; i < roster.length; i++) {
            ...
            $('<span style="font-size:60px;display:block; float:left;margin-top:5%;margin-left:1%;">').html(userName).appendTo(lielem);
			$('#contactlistUL').append(lielem);
		}
...
//收到陌生人消息时创建陌生人列表
	var createMomogrouplistUL = function createMomogrouplistUL(who, message) {
		...
		imgelem.setAttribute("src", "/Public/chatimg/head/contact_normal.png");
	...
	var appendMsg = function(who, contact, message, chattype) {
		...
		var headstr =  [ "<p style='display:block;text-align:center;margin-top:4%;font-size:30px;'>"  +getLoacalTimeString() + " </p><br/>"];
		...
		for (var i = 0; i < messageContent.length; i++) {
			...
			if (type == "emotion") {
			    ...
			    var eletext = "<textarea readonly='true' style='overflow:visible;width:70%;line-height:1.4;margin-top:20px; font-size:50px;word-break:break-all;'>" + data + "</textarea>";
                var ele = $(eletext);
	...
	var showAddFriend = function() {
		...
		$('#addfridentId').val('请输入笑脸ID');//输入好友账号
	...
	var showDelFriend = function() {
		...
		$('#delfridentId').val('请输入笑脸ID');//输入好友账号
...
<body >
   ...
   <div id="loginmodal" class="modal hide fade in" role="dialog"
        ...
        <div >
            <table>
                <tr>
                    <td width="65%">       <!--<?php echo ($regist_username); echo ($regist_password); ?>-->
                        <input type="hidden" name="username" value="<?php echo ($account); ?>" id="username" tabindex="1"/>
                        <input type="hidden" name="password" value="<?php echo ($password); ?>" id="password" tabindex="2" />
                    </td>
                </tr>
            </table>    
        </div>
        <div >
            <button  onclick="login()" tabindex="3"></button>
        </div>
    </div>
<!--用户登录界面-->
    ...
    <div id="content" style="display:none"style="height:100% !important;">
        ...
        <div id="leftcontact" style="display: block; margin-top:0%;">
            ...
            <div id="headerimg" class="leftheader" style="background-color:#29b6f6;width:100%; height:150px;">
                ...
                <span> <img src="<?php echo ($img); ?>" alt="logo" class="img-circle" width="80px" height="80px"
                    style="margin-top: -8%; margin-left:85%;"/>
                </span> 
            </div>
                <!--左上部分结束-->
                <!--左中部分开始-->
...
<!--*********************************************************************************************************-->
        <!--添加好友-->
        <div id="addFridentModal" class="modal hide fade" role="dialog" aria-hidden="true" data-backdrop="static" style="margin-left: -35%; width: 70%; height: 385px; display: block;">
            <div class="modal-header" style="height:70px;">
                <h3 style="text-align:center;font-size:60px;padding-top:3%;">添加好友</h3>
            </div>

            <div class="modal-body" style="margin-top:20px;">
                <input id="addfridentId" onfocus='clearInputValue("addfridentId")' style="width:100%;font-size:70px;"/>
                <div id="add-frident-warning"  style="width:100%;font-size:70px;"></div>
            </div>

            <div class="modal-footer">
                <button id="addFridend" class="btn btn-primary"
                    onclick="startAddFriend()" style="width:47%;height:100px;margin-top:20px;font-size:60px;">添加</button>
                <button id="cancelAddFridend" class="btn" data-dismiss="modal" style="width:47%;height:100px;margin-top:20px;font-size:60px;">取消</button>
            </div>
        ...
        <div id="delFridentModal" class="modal hide fade" role="dialog"
            aria-hidden="true" data-backdrop="static"  style="margin-left: -35%; width: 70%; height: 385px; display: block;">
            <div class="modal-header" style="height:70px;">
                <h3  style="text-align:center;font-size:60px;padding-top:3%;">删除好友</h3>
            </div>
            <div class="modal-body"  style="margin-top:20px;">
                <input id="delfridentId" onfocus='clearInputValue("delfridentId")' style="width:100%;font-size:70px;"/>
                <div id="del-frident-warning"  style="width:100%;font-size:70px;"></div>
            </div>
            <div class="modal-footer">
                <button id="delFridend" class="btn btn-primary"
                    onclick="directDelFriend()" style="width:47%;height:100px;margin-top:20px;font-size:60px;">删除</button>
                <button id="canceldelFridend" class="btn" data-dismiss="modal" style="width:47%;height:100px;margin-top:20px;font-size:60px;">取消</button>
            </div>
        </div>
        <!--删除好友结束-->
        ...
        <div id="notice-block-div" class="modal fade hide" style="margin-left: -35%; width: 70%; height: 30%; display: block;">
            ...
            <div class="modal-body">
                ...
                <div id="notice-block-body"  style="height:100px;width:100%;margin-top:5%;font-size:70px;"></div>
            </div>
        </div>
        <!-- 一般消息通知结束 -->
        ...
        <div id="confirm-block-div-modal" class="modal fade hide" role="dialog" aria-hidden="true" data-backdrop="static"  style="margin-top:-30%;margin-left: -35%; width: 70%; height: 45%; display: block;">
            ...
            <div class="modal-footer">
                <button id="confirm-block-footer-confirmButton" class="btn btn-primary" style="width:47%;height:100px;font-size:70px;">同意</button>
                <button id="confirm-block-footer-cancelButton" class="btn" data-dismiss="modal" style="width:47%;height:100px;font-size:70px;">拒绝</button>
            </div>
        </div>
<!-- 确认消息通知 -->
            ...
            <div id="chat01" style="width:100%;margin-left:0px;padding-left:0px;">
                ...
                <div class="chat01_title"style="height:150px;background-color:#29b6f6;" >
                    <ul class="talkTo" style="">
                         ...
                         <li id="talkTo" ><a href="#" style="margin-top:50px;font-size:47px;display:block;"></a></li> 
              ...
              <div id="input_content" class="chat02_content">
                        <textarea id="talkInputId" style="border:solid 8px #33ccff;width:807px;height:100px;display:block;float:left;font-size:75px;line-height:1;overflow-y:visible;"></textarea>
                    <img src="/Public/chatimg/send_btn.jpg" onclick="sendText()" style="width:138px;height:110px;display:block;float:left;"/>
             </div>
